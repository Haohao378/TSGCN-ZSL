...
def compute_resnet_centers(res_path, classes_txt_path, target_wnids):
    print(f'Loading ResNet features from: {res_path}')
    print(f'Loading Class Mapping from: {classes_txt_path}')
    if not os.path.exists(res_path):
        raise FileNotFoundError(f'ResNet file missing: {res_path}')
    res_mat = sio.loadmat(res_path)
    features = res_mat['features']
    ...
    with open(classes_txt_path, 'r') as f:
        for idx, line in enumerate(f):
            ...
            if len(parts) >= 2 and parts[0].isdigit():
                name_to_label_idx[parts[1]] = int(parts[0])
            else:
                name_to_label_idx[parts[0]] = idx + 1
    centers = []
    print('Computing class centers...')
    for wnid in target_wnids:
        if wnid not in name_to_label_idx:
            alt_wnid = wnid.replace('+', '-') if '+' in wnid else wnid.replace('-', '+')
            if alt_wnid in name_to_label_idx:
                wnid = alt_wnid
            else:
                print(f"Warning: Class '{wnid}' not found in classes.txt! Filling zero.")
                centers.append(np.zeros(2048))
                continue
        label_idx = name_to_label_idx[wnid]
        indices = np.where(labels == label_idx)[0]
        if len(indices) == 0:
            print(f"Warning: No samples found for class '{wnid}' (Label ID: {label_idx})")
            centers.append(np.zeros(2048))
        else:
            class_feats = features[indices]
            mean_feat = np.mean(class_feats, axis=0)
            centers.append(mean_feat)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--suffix', type=str, default='', help='suffix for saved file name')
    parser.add_argument('--method', type=str, default=None, help='override method: DTVSc, etc.')
    parser.add_argument('--use_resnet', action='store_true', help='Enable ResNet-101 ablation mode')
    parser.add_argument('--resnet_path', type=str, default=None, help='Override ResNet .mat file path')
    ...
    if 'CUB' in config['experiment'] or 'SUN' in config['experiment'] or 'FLO' in config['experiment'] or ('AWA2' in config['experiment']):
    ...
    use_resnet = True
    if use_resnet:
        res_path = args.resnet_path if args.resnet_path else config['data'].get('resnet_path')
        classes_txt_path = config['data']['classes_dir']
        train_centers = compute_resnet_centers(res_path, classes_txt_path, train_wnids)
        test_centers = compute_resnet_centers(res_path, classes_txt_path, test_wnids)
        VC = np.concatenate([train_centers, test_centers], axis=0)
    else:
